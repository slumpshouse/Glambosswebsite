...
function searchCustomers(rows, query) {
...
function upsertCustomerByPhone(rows, input) {
...
function calculateLifetimeValue(rows, customerId) {
  return rows
    .filter((row) => row.customerId === customerId)
    .reduce((sum, row) => sum + row.totalPrice, 0);
}

function calculatePurchaseFrequency(rows, customerId) {
...
describe("customer management stubs", () => {
  ...
  it("customer deduplication: unique phone prevents duplicate customer", () => {
    const rows = [{ id: 1, name: "Ava", phone: "555-1111" }];
